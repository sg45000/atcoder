...
def nap_sack(st, op, select, is_end, i=0):
    """
    st: 状態を管理する配列
    op: 値の計算
    select:  状態の遷移先が合流した時どちらを選ぶかの2項演算
    is_end: 終了条件
    """
    if is_end(i):
        return st
    new_st = [0] * (W + 1)  # 新しい配列を作る
    w, v = get_ints()
    for j in range(len(new_st)):
        if j < w:
            new_st[j] = st[j]
        else:
            # ここで選ぶか選ばないかを判定する
            new_st[j] = select(op(st[j - w], v), st[j])
    return nap_sack(new_st, op, select, is_end, i + 1)


ans = nap_sack([0] * (W + 1), add, max, lambda x: x == N)
print(max(ans))
