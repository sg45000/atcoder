...
def get_undirected_graph(n: int, e: int) -> Graph:
    """
...
def bfs(
    n: int, v0s: List[int], g: Graph, initial: int, operate: Callable[[int], int]
) -> List[int]:
    """
    n: ノード総数
    v0s: 最初に探索するノードのリスト
    g: グラフ
    initial: 探索結果のリストの初期値
    operate: 探索時に探索結果をどう更新するか
    """
    ...
    while q:
        v = q.popleft()
        for nex in g[v]:
            if initial != visited[nex]:
                continue
            visited[nex] = operate(visited[v])
            q.append(nex)
...
def dfs(i: int, p=-1):
    for nex in G[i]:
        if nex == p:
            continue
        dfs(nex, i)
        dp[i][0] = (dp[i][0] * ((dp[nex][0] + dp[nex][1]) % MOD)) % MOD
        dp[i][1] = (dp[i][1] * dp[nex][0]) % MOD
    return dp
