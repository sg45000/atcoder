...
B = get_ints_n_lines(N)

G = NetWorkFlowGraph(2 * N + 2)
...
for i in range(N):
    for j in range(8):
        ...
        p = (x + T * dx, y + T * dy)
        if p in BM:
            G.add_edge(i + 1, BM[p], 1)

for i in range(N):
    G.add_edge(0, i + 1, 1)
    G.add_edge(N + i + 1, 2 * N + 1, 1)

max_flow, parent = G.FordFulkerson(0, 2 * N + 1)
...
if max_flow < N:
    ...
    for i in range(N + 1, 2 * N + 1):
        if G.graph[parent[i]][i] == 0:
            x, y = A[parent[i] - 1]
            for j in range(8):
                dx, dy = D[j]
                if [x + T * dx, y + T * dy] == B[i - N - 1]:
                    ans[parent[i] - 1] = j + 1
    print(*ans)
