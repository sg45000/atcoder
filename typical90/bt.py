...
grid = get_char_grid((0, 0), (H - 1, W - 1), "")


ans = -1


def dfs(s, g, visited, p):
    global ans
    for h, w in next_in_grid_range(grid, s, vertical_dist()):
        if (h, w) == g and p != g:
            ans = max(ans, visited[s[0]][s[1]])
        if grid[h][w] == "#" or visited[h][w] > 0:
            continue

        visited[h][w] = visited[s[0]][s[1]] + 1
        dfs((h, w), g, visited, s)
        visited[h][w] = 0
