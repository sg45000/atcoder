...
def calc_area_from_acc_2d(
    acc_2d: List[List[int]], shape: Tuple[int, int, int, int]
) -> int:
    """
    2次元累積和から面積を計算する
    """
...
ans = 0
while count_digit(L) < count_digit(R):
    ans += ((sum_arithmetic_progression(L, 10**d - 1, 10**d - L) % MOD) * d) % MOD
    ans %= MOD
    L = 10**d
    d = count_digit(L)

ans += ((sum_arithmetic_progression(L, R, R - L + 1) % MOD) * d) % MOD

print(ans % MOD)

# (10**18 + int("9" * 19)) * (int("9" * 19) - 10**18 + 1) // 2 % (10**9 + 7)
