...
#############################
N, P, K = get_ints()
A = get_ints_n_lines(N)


def ret_x_if_minus(a, x):
    return a if a >= 0 else x


def warshall_floyd(n: int, d: List[List[int]], x: int):
    V = n
    # kを経由した場合の配列を作る
    ...
    for k in range(V):
        for i in range(V):
            for j in range(V):
                cp_d[i][j] = min(
                    ret_x_if_minus(cp_d[i][j], x),
                    ret_x_if_minus(cp_d[i][k], x) + ret_x_if_minus(cp_d[k][j], x),
                )
    return cp_d


def count(n: int, d: List[List[int]], x: int):
    wf = warshall_floyd(n, d, x)
    cnt = 0
    for i in range(N - 1):
...
min_cnt = count(N, A, P + 1)

if min_cnt == K:
    print("Infinity")
    exit()

if min_cnt > K or count(N, A, 1) < K:
    print(0)
    exit()
