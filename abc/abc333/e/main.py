...
#############################
N = int(input())
TX = get_ints_n_lines(N)

# クエリを逆から見る
potion_uses = []
wants = [0] * (N + 1)  # 欲しいポーションの数を種類ごとに管理
for i in reversed(range(N)):
    t, x = TX[i]
    if t == 1:
        if wants[x] > 0:
            potion_uses.append(1)
            wants[x] -= 1
        else:
            potion_uses.append(0)
    else:
        wants[x] += 1


potion_uses = list(reversed(potion_uses))
if all(map(lambda x: x == 0, wants)):  # wantsが全部捌ければOK
    max_p = 0
    total = 0
    i = 0
    # ポーションを最大何個持っていたかを計算
