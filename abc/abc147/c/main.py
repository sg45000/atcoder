...
#     x, y = xy
#     x -= 1
#     return (b & 1 << (N - x - 1)), y  # 仮定と正直者の証言があっている
...
for b in range(2**N):
    ok = 0
    for i in range(N):
        # print(b & 1 << i, ax[i], list(map(lambda xy: f(b, xy), ax[i])))
        if b & 1 << i and all(map(lambda xy: f(b, xy), ax[i])):  # iが正直者だったら
            ok += 1
    # print(bin(b), ok)
    if b.bit_count() == ok:
        ans = max(ans, ok)
