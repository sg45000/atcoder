...
from typing import Any, Callable, Generic, List, Set, Tuple, TypeVar
...
Weight = int
V = int
WeightedGraph = List[List[Tuple[Weight, V, Any]]]


def dijkstra(n: int, start: int, g: WeightedGraph):
    """
    普通のダイクストラ
    n: 頂点数
    start: 探索始める頂点
    ...
    while q:
        current_w, current_v, road = heapq.heappop(q)
        if d[current_v][0] < current_w:
            continue

        d[current_v] = (current_w, road)
        for next_w, next_v, next_road in g[current_v]:
            new_distance = current_w + next_w
            if new_distance < d[next_v][0]:
                d[next_v] = (new_distance, next_road)
                heapq.heappush(q, (new_distance, next_v, next_road))
    return d
...
for i in range(M):
    a, b, c = ABC[i]
    G[a - 1].append((c, b - 1, i + 1))
    G[b - 1].append((c, a - 1, i + 1))

D = dijkstra(N, 0, G)
print(*list(map(lambda x: x[1], D[1:])))
