...
R, P = topological_sort_tree(N, G, 0)
...
for i in R:
    for j in G[i]:
        if j != P[i]:
            c[i] += c[j]
            dis_sum[i] += c[j] + dis_sum[j]

print(c)
print(dis_sum)
_c = [0] * N
_dis_sum = [0] * N
count_without = [[0] * N for _ in range(N)]
dis_sum_without = [[0] * N for _ in range(N)]
for i in reversed(R):
    ans = 0
    for j in G[i]:
        if j == P[i]:
            _c[j] += 1 + count_without[j][i]
            _dis_sum[j] += dis_sum_without[j][i]
        else:
            _c[j] = c[j]
            _dis_sum[j] = dis_sum[j]
        ans += _c[j] + _dis_sum[j]
    for j in G[i]:
        for k in G[i]:
            if j != k:
                count_without[i][j] += _c[k]
                dis_sum_without[i][j] += _dis_sum[k]
    print(ans)
