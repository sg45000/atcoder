...
# dp[i][0] = i個目の要素まで見た時、1個目とi個目の数字が異なる場合で、i個目までの数列が条件を満たす通り数
# dp[i][0] = i個目の要素まで見た時、1個目とi個目の数字が同じ場合で、i個目までの数列が条件を満たす通り数
dp = [[0] * 2 for _ in range(N + 1)]

dp[1][1] = M
for i in range(1, N):
    dp[i + 1][0] += dp[i][0] * (M - 2)  # 最初と直前の数字を除いた個数
    dp[i + 1][0] += dp[i][1] * (M - 1)  # 最初の数字を除いた個数
    dp[i + 1][1] += dp[i][0]
    dp[i + 1][0] %= MOD
    dp[i + 1][1] %= MOD
