...
HA, WA = getInts()
grid_a = get_char_grid((1, 1), (HA, WA))
HB, WB = getInts()
grid_b = get_char_grid((1, 1), (HB, WB))
HX, WX = getInts()
grid_x = get_char_grid((1, 1), (HX, WX))

is_black = lambda x: x == "#"

gs_a = normalize_gs(from_grid_gs(is_black, grid_a))
gs_b = normalize_gs(from_grid_gs(is_black, grid_b))
gs_x = normalize_gs(from_grid_gs(is_black, grid_x))

for i in range(-HB, HB + 1):
    for j in range(-WB, WB + 1):
        s = set([(v[0] + i, v[1] + j) for v in gs_b])
        if is_shifted_gs(gs_x, gs_a.union(s)):
            print("Yes")
            exit()
