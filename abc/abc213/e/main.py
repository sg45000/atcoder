...
point = [[INF] * W for _ in range(H)]
point[0][0] = 0
while q:
    u = q.popleft()
    for v in filter(
        lambda x: in_range((0, 0), (H - 1, W - 1), x),
        map(lambda x: (x[0] + u[0], x[1] + u[1]), vertical_dist()),
    ):
        if S[v[0]][v[1]] == "." and point[v[0]][v[1]] > point[u[0]][u[1]]:
            q.appendleft(v)
            point[v[0]][v[1]] = point[u[0]][u[1]]

    for v in filter(
        lambda x: in_range((0, 0), (H - 1, W - 1), x),
        map(lambda x: (x[0] + u[0], x[1] + u[1]), punched_dist),
    ):
        if S[v[0]][v[1]] == "#" and point[v[0]][v[1]] > point[u[0]][u[1]] + 1:
            q.append(v)
            point[v[0]][v[1]] = point[u[0]][u[1]] + 1
