...
def _bfs(start):
    ...
    while q:
        s = q.popleft()
        i, j = s
        for n_i, n_j in list(map(lambda x: (x[0] + i, x[1] + j), vertical_dist())):
            if (
                not in_range((0, 0), (H - 1, W - 1), (n_i, n_j))
                or A[n_i][n_j] == "#"
                or visited[n_i][n_j] > 0
            ):
                continue
            q.append((n_i, n_j))
...
while qq:
    s = qq.popleft()
    e = P[s[0]][s[1]]
    visited = _bfs((s[0], s[1]))
    if 0 < visited[g[0]][g[1]] <= e:
        print("Yes")
        exit()
    for k in range(N):
        if seen[k]:
