...
def next_in_grid_range(
    grid: FlexibleList[FlexibleList[T]],
    s: Tuple[int, int],
...
while q:
    ...
    for j in G[a]:
        ...
        if dist[na] != INF:
            continue
        # tは本来 l + d * v(何列車目) + c なので、 l,cを引くことで何番目の電車に乗るべきかわかる
        # (t - l - c) // d が大きい場合は、 k - 1列車目に乗って到着後 t時間まで待つことになる
        v = min((t - l - c) // d, k - 1)
        # (t - l - c) // d　が負の場合は、前の列車からは乗れないことを表している
        if v >= 0:
            new_time = l + v * d
            heapq.heappush(q, (-new_time, j))
