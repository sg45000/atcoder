...
import sys
from sortedcontainers import SortedSet, SortedList, SortedDict, SortedKeyList
...
for i in range(Q):
    ...
    match query[0]:
        case 1:
            sl.add(query[1])
        case 2:
            key = sl.bisect_right(query[1])
            if key - query[2] >= 0:
                print(sl[key - query[2]])
            else:
                print(-1)
        case 3:
            key = sl.bisect_left(query[1])
            if key + query[2] - 1 < len(sl):
                print(sl[key + query[2] - 1])
            else:
                print(-1)
