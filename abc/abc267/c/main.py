...
#############################
"""
ウィンドウが1つずつ移動していく時の差を考える
A = [5, 4, -1, 8]

B[0:2] = A[0] + 2 * A[1]
B[1:3] = B[0:2] - A[0] - A[1] + 2 * A[2]
B[2:4] = B[1:3] - A[1] - A[2] + 2 * A[3]

になる。
部分列Bは直前の部分列Bの計算結果からO(1)で計算できることがわかる

なので、直前の部分列Bとの差を事前に計算しておくことでO(N)で解くことができる
...
diff = []
for i in range(N - M):
    diff.append((A[i + M] * M) - (ACC[i + M] - ACC[i]))
