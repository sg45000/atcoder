...
# 2次元累積和を作る
G = [[[0] * N for __ in range(W + 1)] for _ in range(H + 1)]
...
for i in range(H):
    for j in range(1, W):
        for k in range(N):
            G[i + 1][j + 1][k] = G[i + 1][j + 1][k] + G[i + 1][j][k]

for j in range(W):
    for i in range(1, H):
        for k in range(N):
            G[i + 1][j + 1][k] = G[i + 1][j + 1][k] + G[i][j + 1][k]

ans = [[0] * (W - w + 1) for _ in range(H - h + 1)]
for i in range(H - h + 1):
    for j in range(W - w + 1):
        a = 0
        S = sum_acc2d(G[i][j], G[i + h][j], G[i][j + w], G[i + h][j + w])
        for k in range(N):
            if G[H][W][k] != 0 and S[k] != G[H][W][k]:
                a += 1
        ans[i][j] = a
