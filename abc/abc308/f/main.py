...
P.sort()
# 割引額をマイナスにしてるのは優先度月キューで割引額の高いものから優先してとりたいから
coupons = [(l, -d) for l, d in zip(L, D)]
coupons.sort(key=lambda x: x[0], reverse=True)  # 高い順にクーポンをソート

q = []
heapq.heapify(q)

for p in P:
    # 商品適用最低額が安いクーポンからみていく
    while coupons and p >= coupons[-1][0]:
        c = coupons.pop()
