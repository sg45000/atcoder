...
def bin_search(left, right):
    ...
    if left + 1 >= right:
        i = bisect.bisect_right(A, left)
        j = bisect.bisect_left(B, left)
        return left if i >= M - j else right
    ...
    if i >= M - j:
        ans = min(ans, x)
        return bin_search(left, x)
    elif i < M - j:
        return bin_search(x, right)


left = min(min(A), min(B))
