...
from itertools import chain, combinations, count, permutations, product
...
def power_set(A: List[T]) -> Tuple[T]:
    R = []
    n = len(A)
    for k in range(n + 1):
...
ans = 0
for a in range(1, M + 1):  # aは全探索
    for b in range(1, M // a + 1):  # aとbをかけた値がMを超えた場合はbの対象にはならない
        c = a * b
        ans += B[a] * B[b] * B[c]
print(ans)
