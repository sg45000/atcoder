...
# トポロジカルソートして葉から辿れるようにする
G = [[] for _ in range(N)]
indeg = [0] * N
for i in range(N - 1):
    G[i + 1].append(B[i] - 1)
    indeg[B[i] - 1] += 1
R = topological_sort(N, G, indeg)

# dpの初期化。葉だけ[max, min] を [0, 0] で初期化
dp = [[0, 0]] * N

for i in B:
    dp[i - 1] = [-INF, INF]

# dpの遷移。子から親に遷移して、値を渡していく
for r in R:
    if r == 0:
        continue
    dp[B[r - 1] - 1][0] = max(dp[B[r - 1] - 1][0], dp[r][0] + dp[r][1] + 1)
    dp[B[r - 1] - 1][1] = min(dp[B[r - 1] - 1][1], dp[r][0] + dp[r][1] + 1)

print(dp[0][0] + dp[0][1] + 1)
