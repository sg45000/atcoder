...
#############################
N = int(input())
TXA = get_ints_n_lines(N)


def move(a):
    return [x for x in [a - 1, a, a + 1] if 0 <= x <= 4]
...
for i in range(MAX_T):
    ...
    for j in range(5):
        new_st[j] = max(
            [st[k] for k in move(j)]
        )  # 前の状態から遷移 隣り合う場所からjに遷移できる
        if (i + 1) in M:  # i+1秒目にモグラが出てくるか判定
            x, a = M[i + 1]
            if j == x:  # 今見ている穴がモグラが出てくる穴であればaを足す
                new_st[x] += a
    st = new_st
