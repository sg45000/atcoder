...
N = int(input())
M = ceil(pow(N, 1 / 3))
_, a_primes = eratosthenes(10**3 + 1)
a_primes = [p**2 for p in a_primes]
_, b_primes = eratosthenes(10**4 + 1)
_, c_primes = eratosthenes(10**6 + 1)
c_primes = [p**2 for p in c_primes]

ans = 0
for i in range(len(a_primes)):
    a2 = a_primes[i]
    for j in range(i + 1, len(b_primes)):
        b = b_primes[j]
        max_c2 = N / (a2 * b)
        if max_c2 <= b**2:
            continue
        c_i = bisect.bisect_right(c_primes, max_c2)
        ans += c_i - (j + 1)
