...
def binary_search(left, right):
    if right - left <= 1:
        return left
    m = (left + right) // 2  # m = 何周するか
    total = 0  # total = 何個りんごを食べたか

    for i in range(N):
        total += min(A[i], m)

    if K == total:  # ちょうどm周するとちょうどK個りんごを食べることができる
        return m
    if K < total:  # m周するとK個を超えてりんごを食べる
        return binary_search(left, m)  # m周以下でもう一度二分探索
    elif K > total:  # m周するとK個より少なくりんごを食べる
        return binary_search(m, right)  # m周以上でもう一度二分探索


def calc_rest(consumed):
    return [A[i] - consumed[i] for i in range(N)]


m = binary_search(0, K + 1)

consumed = [min(A[i], m) for i in range(N)]
total_consumed = sum(consumed)
if K == total_consumed:
    print(*calc_rest(consumed))
else:
    for i in range(N):
        if total_consumed < K and consumed[i] + 1 <= A[i]:
            consumed[i] += 1
            total_consumed += 1
