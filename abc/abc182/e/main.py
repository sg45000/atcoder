...
CD = get_ints_n_lines(M)

# ビットで管理 -1=ブロック,0=何もなし, 1=縦方向の光が届く, 2=横方向の光が届く, 3=縦横両方の光が届く
...
for s in range(N):
    a, b = AB[s]
    a, b = a - 1, b - 1
    q = deque([(a, b, (0, 1)), (a, b, (1, 0)), (a, b, (-1, 0)), (a, b, (0, -1))])
    while q:
        i, j, dist = q.popleft()
        u, v = i + dist[0], j + dist[1]
        if not in_range((0, 0), (H - 1, W - 1), (u, v)):
            continue
        if visited[u][v] == -1:
            continue
        if (visited[u][v] & 1) and dist[0] != 0:  # 縦方向
            continue
        if ((visited[u][v] >> 1) & 1) and dist[1] != 0:  # 横方向
            continue

        visited[u][v] |= abs(dist[0]) | (abs(dist[1]) << 1)
        q.append((u, v, dist))
ans = 0
for i in range(H):
    for j in range(W):
