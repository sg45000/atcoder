...
N = int(input())
A = get_char_grid((1, 1), (N, N), trim_string=" ")
B = get_char_grid((1, 1), (N, N), trim_string=" ")
A_gs = from_grid_gs(lambda x: x == "1", A)
B_gs = from_grid_gs(lambda x: x == "1", B)

for _ in range(4):
    A_gs = rotates_gs(A_gs, N)
    if is_contain_gs(A_gs, B_gs):
        print("Yes")
        exit()
