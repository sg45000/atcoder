...
# dp[iビット目][Aの要素の組み合わせ] = 条件を満たすxの個数
dp = [[0] * (1 << N) for _ in range(D + 1)]
dp[0][0] = 1

for i in range(1, D + 1):
    m = 0  # mのjビット目 = A[j]のiビット目
    for j in range(N):
        if A[j] & (1 << (i - 1)) != 0:
            m |= 1 << j

    # あるAの組み合わせbitsでは、それぞれの要素のiビット目まで見た時、条件を満たすxが◯個ある
    for bits in range(1 << N):  # bits は Aの要素の組み合わせを表現
        dp[i][bits] += dp[i - 1][bits]
        dp[i][bits | m] += dp[i - 1][bits]
