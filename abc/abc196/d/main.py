...
#############################
"""
DFSで全探索(O(10**7~8)くらい？)
1畳のたたみの置き方の組み合わせだけ考えれば良い

"""
H, W, A, B = get_ints()

grid = range_to_coords((1, 1), (H, W))
S = set(grid)
...
def dfs(i, s: Set):
    ...
    for c in list(s):
        ...
        s.remove(c)
        h_c = (h + 1, w)
        if h + 1 <= H and h_c in s:
            dfs(i - 1, remove_from_set(s, h_c))
        w_c = (h, w + 1)
        if w + 1 <= W and w_c in s:
            dfs(i - 1, remove_from_set(s, w_c))
