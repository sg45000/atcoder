...
dp = [[[None] * 61 for _ in range(61)] for _ in range(61)]
...
for i in range(60):
    for x in range(61):
        for y in range(61):
            ...
            X, Y = dp[i][x][y]
            if C >> i & 1:
                dp[i + 1][x + 1][y] = (X + (1 << i), Y)
                dp[i + 1][x][y + 1] = (X, Y + (1 << i))
            else:
                dp[i + 1][x][y] = dp[i][x][y]
                dp[i + 1][x + 1][y + 1] = (X + (1 << i), Y + (1 << i))

print(-1) if dp[-1][a][b] is None else print(*dp[-1][a][b])
