...
#############################
"""
選んだ個数ごとに独立で和としてありえる数の個数を数えて足していけばいい
i個の数を選ぶ時、和の最小値から最大値の間の数であれば必ず作ることができる
(和を1つずつ増やすことが可能だから)

i個の数を選ぶときは
最小値は 小さい順で先頭からi個選んだ和
最大値は 末尾からi個選んだ和 となる

i個選んだときは 最大値 - 最小値 + 1 個の和を作ることが可能
"""
...
for i in range(1, N + 1):
    ACC_ASC.append(i + ACC_ASC[-1])

ACC_DESC = [N]
for i in reversed(range(0, N)):
    ACC_DESC.append(i + ACC_DESC[-1])

ans = 0
for i in range(K, N + 2):  # K個から N + 1個まで選んだとき
    maximum = ACC_DESC[i - 1]
    minimum = ACC_ASC[i - 1]
    ans += (maximum - minimum + 1) % MOD
    ans %= MOD
