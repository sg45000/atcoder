...
get_chars_with_trim = lambda s: lambda: list(input().split(s))
...
) -> FlexibleList[FlexibleList[str]]:
    """_summary_
    文字で構成されたグリッドを読み込み2次元配列で返す
    """
    (ulh, ulw) = upper_left
    (lrh, lrw) = lower_right
    getter = get_chars_with_trim(trim_string) if trim_string else get_chars
    return FlexibleList(
        [FlexibleList(getter(), (ulw, lrw)) for _ in range(lrh + 1 - ulh)],
        (ulh, lrh),
    )


def get_char_grid_with(
    upper_left: Coordinate,
    lower_right: Coordinate,
    f: Callable[[str], T],
    trim_string: str | None,
) -> FlexibleList[FlexibleList[T]]:
    """_summary_
    文字で構成されたグリッドを読み込み2次元配列で返す
    """
    (ulh, ulw) = upper_left
    ...
    for i in range(ulh, lrh + 1):
        for j in range(ulw, lrw + 1):
            grid[i][j] = f(grid[i][j])
    return grid
...
def get_horizontal_line_gs(gs: GridSet, line_no: int) -> GridSet:
    """
    指定して行Noを直線に切り取る
    """
    ret = set()
...
def get_vertical_line_gs(gs: GridSet, line_no: int) -> GridSet:
    """
    指定して列Noを直線に切り取る
    """
    ret = set()
...
grid = get_char_grid_with(
    (0, 0), (N - 1, N - 1), lambda x: 1 if x == "B" else 0, trim_string=None
)

ACC = [[0] for _ in range(N + 1)]
for i in range(N):
    for a in list(accumulate(grid[i])):
        ACC[i].append(a)
