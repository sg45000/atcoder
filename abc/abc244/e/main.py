...
# Main
#############################
N, M, K, S, T, X = get_ints()
N, M, K, S, T, X = N, M, K, S - 1, T - 1, X - 1
MOD = 998244353
...
for _ in range(K):
    ...
    for i in range(N):
        for e in G[i]:
            # Xの頂点を新たに追加する場合はXの個数が奇数 - 偶数間で状態遷移する
            next_st[i][0] += st[e][1 if i == X else 0]
            next_st[i][0] %= MOD
            next_st[i][1] += st[e][0 if i == X else 1]
            next_st[i][1] %= MOD
