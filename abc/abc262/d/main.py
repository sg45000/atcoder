...
for i in range(1, N + 1):  # iで割る
    dp = [[[0] * (i + 1) for _ in range(i + 2)] for _ in range(N + 1)]
    dp[0][0][0] = 1
    for j in range(N):  # 先頭j項以降から選ぶ
        for k in range(i + 1):  # k個選ぶ
            for l in range(i):  # 先頭j項以降からk個選んだ総和をiで割った余りを求める
                dp[j + 1][k][l] += dp[j][k][l]
                dp[j + 1][k][l] %= MOD
                if k != i:
