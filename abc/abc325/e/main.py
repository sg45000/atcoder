...
#############################
N, A, B, C = get_ints()
D = get_ints_n_lines(N)

G_car = [[] for _ in range(N)]

for i in range(N):
    for j in range(N):
        G_car[i].append((D[i][j] * A, j))


G_train = [[] for _ in range(N)]

for i in range(N):
    for j in range(N):
        G_train[i].append((D[i][j] * B + C, j))


# 各頂点から全ての頂点に辺があるような場合は O(V^2log(V))
min_car = dijkstra(N, 0, N - 1, G_car)
min_train = dijkstra(N, N - 1, 0, G_train)


ans = INF
for c, t in zip(min_car, min_train):
    ans = min(c + t, ans)
