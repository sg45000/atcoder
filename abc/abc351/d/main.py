...
def dijkstra(n: int, start: int, end: int, g: WeightedGraph) -> List[int]:
    """
    普通のダイクストラ
    n: 頂点数
    start: 探索始める頂点
...
for i in range(H):
    for j in range(W):
        if S[i][j] == ".":
            p = 0
            for vi, vj in vertical_dist():
                ui, uj = vi + i, vj + j
                if in_range((0, 0), (H - 1, W - 1), (ui, uj)):
                    if S[ui][uj] == "#":
                        p += 1
            if p == 0:
                R[i][j] = 1
        if S[i][j] == "#":
            for vi, vj in vertical_dist():
                ui, uj = vi + i, vj + j
                if in_range((0, 0), (H - 1, W - 1), (ui, uj)) and S[ui][uj] == ".":
                    leaves.add((ui, uj))
...
def dfs(x, s):
    ...
    while stack:
        ...
        for vi, vj in vertical_dist():
            ui, uj = vi + i, vj + j
            if in_range((0, 0), (H - 1, W - 1), (ui, uj)) and visited[ui][uj] != s:
                visited[ui][uj] = s  # ここで訪問済みにする
                stack.append((ui, uj))
                p += 1
