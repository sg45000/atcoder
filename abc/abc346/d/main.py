...
# 状態は st[良い文字列かどうか][0で終わってるか1で終わってるか] = コストの最小値
def make_st():
    return [[INF] * 2 for _ in range(2)]
...
for i in range(N):
    ...
    if i == 0:
        ...
        if s == 0:
            next_st[0][0] = st[0][1]
            next_st[1][0] = min(st[0][0], st[1][1])
            next_st[0][1] = st[0][0] + c
            next_st[1][1] = min(st[0][1] + c, st[1][0] + c)
        else:
            next_st[0][0] = st[0][1] + c
            next_st[1][0] = min(st[0][0] + c, st[1][1] + c)
            next_st[0][1] = st[0][0]
            next_st[1][1] = min(st[0][1], st[1][0])
    st = next_st
