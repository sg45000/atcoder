...
G = {"s": "n", "n": "u", "u": "k", "k": "e", "e": "s"}


def dist(h, w):
    d = [(h + 1, w), (h, w + 1), (h - 1, w), (h, w - 1)]
    return list(filter(lambda x: 0 <= x[0] < H and 0 <= x[1] < W, d))
...
while q:
    s = q.popleft()
    h, w = s
    for nh, nw in dist(h, w):
        if S[h][w] in G and G[S[h][w]] == S[nh][nw] and not visited[nh][nw]:
            q.append((nh, nw))
            visited[nh][nw] = True
